...
| 상품 등록 | Create Product | 비속어가 포함되지 않은 상품 이름과 0이상의 상품 가격을 가진 상품을 키친포스에 등록하는 행위 |
| 비속어   | Profanity      | 상풍 이름에 포함할 수 없는 욕설 또는 비속어                            |
| 상품 목록 | Product List   | 0~N 개의 상품을 모은 목록                                     |
...
| 메뉴 그룹 목록 | Menu Group List    | 0~N 개의 메뉴 그룹을 모은 목록                                         |
...
| 메뉴 가격    | Menu Price         | 메뉴의 가격. 메뉴는 0이상이어야 하고 해당 메뉴에 속한 메뉴 상품 금액의 합보다 클 수 없음.                           |
...
| 메뉴 가격 변경 | Change Menu Price  | 메뉴 가격을 변경하는 행위. 메뉴 가격 변경 시 메뉴 가격은 메뉴는 0이상이어야 하고 해당 메뉴에 속한 메뉴 상품 금액의 합보다 클 수 없음. |
| 메뉴 목록    | Menu List          | 0~N 개의 메뉴를 모은 목록                                                                |
...
| 주문 테이블 목록  | Order Table List         | 0~N 개의 주문 테이블을 모은 목록                        |
